        ...
        for i in range(0, len(all_songs)):
            ...
            for j in range(0, len(user_songs)):
                ...
                if len(users_intersection) != 0:
                    ...
                    users_union = users_i.union(users_j)    # 计算并集

                    cooccurence_matrix[j, i] = float(len(users_intersection)) / float(len(users_union)) # 对应位置更新数据，Jaccard相似系数来进行衡量
                else:
                    cooccurence_matrix[j, i] = 0

        return cooccurence_matrix   # 返回最终的矩阵
